...
LevelEngine:
  ...
  call  CheckSpecialActions   ;check controls for a special action
  call  CheckFreezeControls   ;At start and at end of game you cant move
  call  PlayerAction          ;players move, jump, punch etc etc / ALSO checks if players get hit
  ...
  call  restorebackgroundAndPutplayerback
  call  playmusic
  call  restorebackgroundAndPutplayerfront
  call  HandleTossCoolDown
...
PlayerAction:
  ;check if player died, then handle PlayerActions every other interrupt (which results in a slowdown of the game)
  ld    a,(P1Dies?)
  ld    hl,P2Dies?
...
HandleSpecial:                ;nc=ground found
  ...
  ld    a,(de)                ;only continue if impact is on the first frame of when enemy gets hit (HeavilyHitAnimationStep is 10 at that point)
  cp    10
  jr    z,.setnextspecial

  .impactfound:
  ld    a,(bc)                ;only continue if impact is on the first frame of when enemy gets hit (HitAnimationStep is 5 at that point)
  cp    5
  ret   nz
...
PlayerDied:
  ...
  jp    z,PlayerAction.endcheckPlayerDied
...
restorebackgroundAndPutplayerback:
...
restorebackgroundAndPutplayerfront:
  ld    a,(PlayerPriority)    ;1=player1   2=player2 (which player comes in front of the other ?)
  dec   a
  jp    z,.P1
  .P2:
